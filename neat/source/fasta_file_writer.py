...
class FastaFileWriter:
    ...
    def write_record(self, sequence, chrom, N_seq_len=0, ignored_ending=0):
        ...
        if sequence is None or N_seq_len > 0:
            strDNA = "N" * N_seq_len
        else:
            strDNA = str(sequence)
    ...
    def close_file(self):
        final_name = self.file[:-len('_temp')]
        os.rename(self.file, final_name)
