...
#         trinuc_snp_bias_per_region = {region: [[0. for _ in range(self.seq_len)] for _ in range(self.ploidy)] for region in Regions}
#         self.trinuc_bias_per_region = {region: [None for _ in range(self.ploidy)] for region in Regions}
#         for region in Regions:
#             self.update_mask_of_region(self.win_buffer, self.seq_len) #TODO implement. set to int 0 if all elements are 0
#             for p in range(self.ploidy):
#                 for i in range(self.win_buffer + 1, self.seq_len - 1):
#                     trinuc_snp_bias_per_region[region][p][i] = self.mask_per_region[region][i] * \
#                         self.models_per_region[region][p][7][ALL_IND[str(self.sequences[p][i - 1:i + 2])]]
#                 self.trinuc_bias_per_region[region][p] = DiscreteDistribution(trinuc_snp_bias_per_region[p][self.win_buffer + 1:self.seq_len - 1],
...
#
#     def init_poisson(self, seq_len=-1):
...
#             ind_l_list_per_region[region] = [seq_len * self.models_per_region[region][i][0] * self.models_per_region[region][i][2] * self.ploid_mut_frac_per_region[region][i] for i in
#                           range(ploids)]
#             snp_l_list_per_region[region] = [seq_len * self.models_per_region[region][i][0] * (1. - self.models_per_region[region][i][2]) * self.ploid_mut_frac_per_region[region][i] for i in
#                           range(ploids)]
#         k_range = range(int(seq_len * MAX_MUTFRAC))
...
#
#         #TODO maybe suffle SNPs and indels and insert in an arbitrary order?
...
#                 random_snps_per_region = max(self.snps_to_add_per_region[region][i], 0)
#                 for j in range(random_snps_per_region):
...
#             if IGNORE_TRINUC:
#                 k = (self.seq_len - 2) - (self.win_buffer + 1) + 1
#                 event_pos = random.choices(range(self.win_buffer + 1, self.seq_len - 1), weights=self.mask_per_region[region][self.win_buffer + 1 : self.seq_len], k=k)
...
#         for region in Regions:
#             for i in range(self.ploidy):
#                 random_indels_per_region = max(self.indels_to_add_per_region[region][i], 0)
#                 for j in range(random_indels_per_region):
...
#     def find_position_indel(self, region, which_ploid):
#         # try to find suitable places to insert indels
#         event_pos = -1 ???
#         for attempt in range(MAX_ATTEMPTS):
#             event_pos = random.randint(self.win_buffer, self.seq_len - 1)
#             k = (self.seq_len - 2) - (self.win_buffer + 1) + 1
#             event_pos = random.choices(range(self.win_buffer + 1, self.seq_len - 1),
#                                        weights=self.mask_per_region[region][self.win_buffer + 1 : self.seq_len], k=k)
#             for p in which_ploid:
#                 if self.black_list[p][event_pos]:
#                     event_pos = -1
#             if event_pos != -1:
...
#     ALL = 'all'
#     #TODO we should iterate over EXON,INTRON,INTERGENIC regions when inserting mutation, without ALL,
#     # or to use ALL alone
#
# class Stats(Enum):
