...
class TaskQueue(Queue):
    _decoded_messages = {} # TODO is it the correct way to make a class member public in python?

    def __init__(self, decode_message, num_workers=1):
        Queue.__init__(self)
    ...
    def add_task(self, m, task_counter):
        self.put((m,task_counter))
    ...
    def worker(self):
        while True:
            m, task_counter = self.get()
            decoded = self._decode_message(m)
            self._decoded_messages[task_counter] = decoded # Dictionary operations are atomic: https://web.archive.org/web/20201108091210/http://effbot.org/pyfaq/what-kinds-of-global-value-mutation-are-thread-safe.htm
            self.task_done()
